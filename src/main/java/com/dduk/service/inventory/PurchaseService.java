...
public class PurchaseService {
    ...
    public PurchaseOrderResponseDto createPurchaseOrder(PurchaseOrderCreateDto requestDto, Long requestedByMemberId) {
        ...
        Member requestedBy = findRequesterMember(requestedByMemberId);
    ...
    private void validateCreateRequest(PurchaseOrderCreateDto requestDto) {
        ...
        for (PurchaseOrderItemCreateDto itemDto : requestDto.getItems()) {
            if (itemDto == null || itemDto.getItemId() == null || itemDto.getQuantity() == null || itemDto.getQuantity() <= 0) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "발주 품목과 수량을 확인해 주세요.");
            }
            if (itemDto.getUnitPrice() == null || itemDto.getUnitPrice().compareTo(BigDecimal.ZERO) < 0) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "발주 단가를 확인해 주세요.");
            }
        }
    }
    ...
    private Member findRequesterMember(Long memberId) {
        if (memberId != null) {
