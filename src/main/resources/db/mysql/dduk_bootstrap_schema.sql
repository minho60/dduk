...
CREATE TABLE IF NOT EXISTS items (
    ...
    KEY idx_items_registered_by_id (registered_by_id),
    CONSTRAINT fk_items_default_vendor
        FOREIGN KEY (default_vendor_id) REFERENCES vendors (id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,
    CONSTRAINT fk_items_registered_by
        FOREIGN KEY (registered_by_id) REFERENCES members (id)
        ON DELETE SET NULL
        ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
SET @drop_purchase_orders_registered_by_fk = (
    SELECT IF(
        COUNT(*) > 0,
        'ALTER TABLE purchase_orders DROP FOREIGN KEY fk_purchase_orders_registered_by',
        'SELECT 1'
    )
    FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE()
      AND TABLE_NAME = 'purchase_orders'
      AND CONSTRAINT_NAME = 'fk_purchase_orders_registered_by'
);
PREPARE drop_purchase_orders_registered_by_fk_stmt FROM @drop_purchase_orders_registered_by_fk;
EXECUTE drop_purchase_orders_registered_by_fk_stmt;
DEALLOCATE PREPARE drop_purchase_orders_registered_by_fk_stmt;

SET @drop_purchase_orders_registered_by_index = (
    SELECT IF(
        COUNT(*) > 0,
        'ALTER TABLE purchase_orders DROP INDEX idx_purchase_orders_registered_by_id',
        'SELECT 1'
    )
    FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'purchase_orders'
      AND INDEX_NAME = 'idx_purchase_orders_registered_by_id'
);
PREPARE drop_purchase_orders_registered_by_index_stmt FROM @drop_purchase_orders_registered_by_index;
EXECUTE drop_purchase_orders_registered_by_index_stmt;
DEALLOCATE PREPARE drop_purchase_orders_registered_by_index_stmt;

SET @drop_purchase_orders_registered_by_column = (
    SELECT IF(
        COUNT(*) > 0,
        'ALTER TABLE purchase_orders DROP COLUMN registered_by_id',
        'SELECT 1'
    )
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'purchase_orders'
      AND COLUMN_NAME = 'registered_by_id'
);
PREPARE drop_purchase_orders_registered_by_column_stmt FROM @drop_purchase_orders_registered_by_column;
EXECUTE drop_purchase_orders_registered_by_column_stmt;
DEALLOCATE PREPARE drop_purchase_orders_registered_by_column_stmt;
