...
async function initReports() {
    ...
    function updateTrialBalanceKPIs(data) {
        ...
        if (status) {
            status.textContent = totalDebit === totalCredit ? '차대 일치' : '차대 불일치';
...
function updateJournalBalances() {
    ...
    if (badge) {
        badge.textContent = isBalanced ? '대차 일치' : '차대 불일치';
    ...
    if (message) {
        if (items.length < 2) {
            message.textContent = '전표 라인은 최소 2개 이상이어야 합니다.';
        } else if (totalDebit === 0 || totalCredit === 0) {
            message.textContent = '차변 또는 대변 합계가 0입니다.';
        } else if (!isBalanced) {
            message.textContent = '차대 금액이 일치하지 않습니다.';
        } else {
            message.textContent = '전표를 생성할 수 있습니다.';
        }
...
window.handleJournalAction = async function handleJournalAction(id, action) {
    ...
    const messages = {
        ...
        cancel: '이 전표를 취소하시겠습니까?',
        delete: '이 전표를 삭제하시겠습니까?'
    };

    if (!confirm(messages[action] || '계속하시겠습니까?')) {
        return;
    }
    ...
    try {
        ...
        showToast('전표 상태가 변경되었습니다.', 'success');
...
async function openJournalModal(id) {
    try {
        ...
        if (!journal) {
            throw new Error('전표 정보를 찾을 수 없습니다.');
        }
...
async function renderPeriodList() {
    try {
        ...
        if (!filtered.length) {
            tbody.innerHTML = '<tr><td colspan="7">표시할 회계 기간이 없습니다.</td></tr>';
            return;
        }
...
window.handlePeriodAction = async function handlePeriodAction(periodKey, action) {
    const sessionLoginId = window.ddukSession?.getSession?.().loginId || 'SYSTEM';
    const messages = {
        close: `이 기간(${periodKey})을 마감하시겠습니까? 마감 후에는 전표 생성과 기표가 불가능합니다.`,
        reopen: `이 기간(${periodKey})을 재오픈하시겠습니까?`
    };
...
async function loadPayrollReferenceHistory() {
    ...
    try {
        ...
        const response = await fetch('/api/v1/admin/tasks?taskType=RPA&actionName=collect_hr_reference&size=5&sort=requestedAt,desc', {
            headers,
            cache: 'no-store'
        });
        if (!response.ok) {
...
function renderPayrollReference(summary) {
    ...
    if (statusBadge) {
        statusBadge.textContent = rpa.status === 'READY' ? '비교 가능' : (rpa.status === 'MISSING_FILE' || rpa.status === 'EMPTY_RESULT' ? '결과 부족' : '대기');
...
async function triggerPayrollReference() {
    ...
    try {
        ...
        showToast(error.message || 'HR 기준 조회 실행 요청에 실패했어.', 'error');
    } finally {
        if (button) {
