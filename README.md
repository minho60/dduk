...
- 목표: 구매/재고, 회계, AI 업무지원, 관리자 기능을 하나의 ERP 흐름으로 통합한다.
- 방향: 백엔드 중심의 업무 시스템에 프론트 UI, AI 보조 기능, 자동화 기능을 단계적으로 확장한다.
- 현재 상태: 공통 인프라 및 저장소 구조를 기반으로 각 도메인 기능을 구현 중이다.
...
| 담당자 | 도메인 | 주요 기능 |
| :--- | :--- | :--- |
| 서우리 | 구매/발주, 문서/증빙, 회계 일부 | 구매/발주 대시보드, 발주 요청, 발주 관리, 입고 등록, 거래처 관리, 세금계산서, 비용 처리, 증빙 업로드, OCR 문서함 |
| 김슬기 | 재고관리, 회계관리 | 재고관리 대시보드, 재고 조회, 입출고 이력, 창고 이동, 자동 발주 추천, 회계 대시보드, 거래 내역 등록, 매입/매출 현황, 월 마감, 급여 계산, 회계 리포트 |
| 김민호 | AI 업무지원, 관리자 | AI 챗봇, 이상 탐지, 예측 분석, 사용자 관리, 권한 관리, 시스템 로그, 서버 모니터링 |
